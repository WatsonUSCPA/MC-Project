...
const Kits: React.FC = () => {
  ...
  // グローバルカートコンテキストを使用
  const { cart, addToCart, removeFromCart, updateKitPrice } = useCart();
  ...
  // 生地購入数が変わった時にカート内のキットを更新
  useEffect(() => {
    if (fabricCount > 0) {
      const cartKits = cart.filter(item => item.productType === 'kit');
      if (cartKits.length > 0) {
        // 無料キット数を再計算
        const newFreeKitCount = getFreeKitCount();
        
        // カート内のキットを更新
        cartKits.forEach((kitItem, index) => {
          const isFree = index < newFreeKitCount;
          const baseName = kitItem.name.replace(' (無料)', '');
          const newName = isFree ? `${baseName} (無料)` : baseName;
          const newPrice = isFree ? '0' : kits.find(k => k.id === kitItem.managementNumber)?.price || kitItem.price;
  ...
  }, [fabricCount, cart, kits, updateKitPrice]);
  ...
  if (loading) {
    return (
      <div className="kits-page-container">
        <h1 style={{ color: 'var(--color-primary)', fontSize: '1.6rem', marginBottom: '0.5rem' }}>🧩 キットページ</h1>
    ...
    <>
      <div className="kits-page-container">
        ...
        <p style={{ fontSize: '1.1rem', color: 'var(--color-text-light)', marginBottom: '1rem', lineHeight: 1.6 }}>
          ...
          <strong style={{ color: 'var(--color-primary)' }}>※ キットは1個ずつしか購入できません</strong>
        </p>
          ...
          <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
            <div style={{ display: 'inline-flex', gap: '0.5rem', flexWrap: 'wrap', justifyContent: 'center' }}>
              <button
                ...
                style={{
                  background: filterLevel === '' ? '#FFD4C4' : 'var(--color-background)',
                  color: filterLevel === '' ? '#E1306C' : 'var(--color-text)',
                 ...
                 return (
                   <button
                     ...
                     onClick={() => {
                      ...
                      className={!isSelected ? 'filter-button-blue' : ''}
                   >
                     {level}
                   </button>
                 );
                ...
                <div key={kit.id} style={{
                  ...
                  {fabricCount > 0 && (
                   ...
                   <button
                     ...
                     style={{
                       width: '100%',
                       background: isKitInCart(kit.id) ? '#f5f5f5' : 'var(--color-primary)',
